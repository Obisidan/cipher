//! SHA-256 hash function (FIPS-180-4). Pure Rust, no external deps.

use crate::bytes::{read_u32_be, write_u32_be};
...
impl Sha256 {
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        if self.buffer_len > 0 {
            let remaining = SHA256_BLOCK_SIZE - self.buffer_len;
        ...
        while offset + SHA256_BLOCK_SIZE <= data.len() {
            process_block(&mut self.state, &data[offset..offset + SHA256_BLOCK_SIZE]);
    ...
    pub fn finalize(mut self) -> [u8; SHA256_DIGEST_SIZE] {
        ...
        if self.buffer_len > 56 {
            // Need an extra block
            for i in self.buffer_len..SHA256_BLOCK_SIZE {
                self.buffer[i] = 0;
            }
            process_block(&mut self.state, &self.buffer);
            self.buffer_len = 0;
        ...
        for i in 0..8 {
            self.buffer[56 + i] = ((bit_len >> (56 - 8 * i)) & 0xFF) as u8;
        }
        ...
        for i in 0..8 {
            write_u32_be(&mut digest[i * 4..(i + 1) * 4], self.state[i]);
        }
        digest
    }
}
...
mod tests {
    ...
    fn test_sha256_two_blocks() {
        // "abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq" — spans 2 blocks
